...
function main() {
  ...
  local zip_filename="${temporary_directory}/exclude file exmaple.zip"

  zip "${zip_filename}" -r "${temporary_directory}" -x "${second_file}"
  if [[ 0 -ne $? ]]; then echo "Unable to write files: ${first_file} to ${zip_filename}" && exit 255; fi
