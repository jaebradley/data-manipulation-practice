...
function main() {
  ...
  local zip_filename="${temporary_directory}/encrypted exmaple.zip"

  zip "${zip_filename}" -P "password" "${first_file}"
  if [[ 0 -ne $? ]]; then echo "Unable to zip encrypted file: ${first_file} to ${zip_filename}" && exit 255; fi
